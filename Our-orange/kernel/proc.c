...
int queue_slice[] = {100, 200, 300};
int queue_len[] = {NR_TASKS+NR_NATIVE_PROCS, 0, 0};
/*****************************************************************************
 *                                schedule
 *****************************************************************************/
/**
...
PUBLIC void schedule()
{
	...
	if (cur->time <= 0)
	{
		...
		for (p = &FIRST_PROC; p <= &LAST_PROC; p++)
			if (p->q == cur->q && p->pos > cur->pos)
				p->pos--;
		cur->q += (cur->q == 2 ? 0 : 1); // 将当前进程重新加入队列
		if(cur-proc_table<NR_TASKS)
            cur->time = queue_slice[cur->q] * 3;
        else
            cur->time = queue_slice[cur->q] * 1;
		...
		{
			if (proc_table[i].q == -1 || proc_table[i].p_flags != 0)
                continue;
	...
	else if (cur->q > 0 && queue_len[0] > 0 || cur->p_flags!=0)
	{
		for (i = 0; i < NR_PROCS; i++)
		{
			if (proc_table[i].q==-1 || proc_table[i].p_flags!=0)
                continue;
			if (proc_table[i].q == cur->q && proc_table[i].pos > cur->pos)
				proc_table[i].pos--;
		}
		int i_next_proc = 0; // 选出优先级最高的队列
		int min_prio = 1000;
